/*******************************************************************************
 *
 * AMP Agent Library - programming interface
 *
...
/**
 * @file amp_agent.h
 * @brief AMP Agent Library - programming interface
 */
...
/*----------------------------------------------------------------------------*/
// AMP Agent Library static configuration. Fixed for a library instance. 
// This mean that for compiled library packaget these are here for information purpose only.
...
#define AMP_CFG_LOG_EXTRAS 0        /**< @brief log file name & line number (0=off, 1=on) */
...
{
	uint64_t (*now)(void);
	void (*param_changed)(amp_param_t *param);
...
/**
 * @brief Check whether param is with given id 
 * @param[in]  _id      param ID 
 * @param[in]  _param   param object reference
 */
#define	AMP_AGENT_PARAM_IS(_id,_param)  (_param->id == _id)

/*----------------------------------------------------------------------------*/
// AMP Agent Library functions
int amp_agent_init(system_info_t *sys_info, system_data_t *sys_data, device_api_t *device_api);
...
int amp_agent_set_log(amp_log_level_t level, const char *prefix);
